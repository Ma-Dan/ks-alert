				...
				if err != nil {
					if err == rpctypes.ErrKeyNotFound {
						if _, err := client.Put(context.TODO(), svcKey, svcAddr, etcd3.WithLease(resp.ID)); err != nil {
							log.Printf("grpclb: set service '%s' with ttl to etcd3 failed: %s", svcName, err.Error())
						}
					} else {
						log.Printf("grpclb: service '%s' connect to etcd3 failed: %s", svcName, err.Error())
					}
				} else {
					...
					if _, err := client.Put(context.Background(), svcKey, value, etcd3.WithLease(resp.ID)); err != nil {
						log.Printf("grpclb: refresh service '%s' with ttl to etcd3 failed: %s", svcName, err.Error())
					}
				}
			}
		}
...
func UnRegister() error {
	...
	if _, err := client.Delete(context.Background(), svcKey); err != nil {
		log.Printf("grpclb: deregister '%s' failed: %s", svcKey, err.Error())
	} else {
		log.Printf("grpclb: deregister '%s' ok.", svcKey)
	}
	return err
}
